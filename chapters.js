...
(() => {
  ...
  // Visible copy is kept free of dash punctuation, including text created by
  // the later recommendation and career scripts. Links and asset URLs stay intact.
  function cleanVisibleCopy() {
    ...
    nodes.forEach(node => { node.nodeValue = node.nodeValue.replace(/[\-–—]/g, ' ' ).replace(/ {2,}/g, ' '); });
    document.body.querySelectorAll('[aria-label],[aria-labelledby]').forEach(element => {
      for (const name of ['aria-label']) {
        if (element.hasAttribute(name)) element.setAttribute(name, element.getAttribute(name).replace(/[\-–—]/g, ' ').replace(/ {2,}/g, ' '));
      }
    });
  }
  window.addEventListener('load', cleanVisibleCopy, { once:true });
  new MutationObserver(cleanVisibleCopy).observe(document.body, { childList:true, subtree:true });
